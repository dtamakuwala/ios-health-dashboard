...
SELECT
  ...
  SUM(DNU) AS new_users,
  SUM(DAU) AS dau,
  SUM(total_survey_revenue) AS survey_revenue,
  SUM(gc_cost) AS gc_cost,
  SUM(pending_gc_cost) AS pending_gc_cost,
  SUM(game_pass_revenue) + SUM(game_pass_renewal_revenue) AS game_pass_revenue,
  SUM(total_lp_revenue) AS install_revenue,
  SUM(iap_install_revenue) AS iap_install_revenue,
  SUM(iaa_install_revenue) AS client_ad_revenue,
  SUM(num_lp_game_installs) AS game_installs,
  SUM(lp_iap_spend_usd) AS lp_iap_spend_usd,
  SUM(total_cumulative_points_balance) AS liabilities_points,
  SUM(gc_points_redeemed) AS pts_redeemed_gc,
  SUM(sweepstakes_points_redeemed) AS pts_redeemed_sweeps,
  SUM(treasure_hunt_points_redeemed) AS pts_redeemed_th,
  SUM(
    quick_games_points_rewarded + game_trivia_points_rewarded + surveys_points_rewarded +
    loyalty_play_points_rewarded + daily_streak_points_rewarded + promo_code_points_rewarded +
    tournaments_points_rewarded + treasure_hunt_points_rewarded + iap_offers_points_rewarded +
    referral_points_rewarded + welcome_bonus_points_rewarded + failed_install_points_rewarded +
    cs_comped_points_rewarded
